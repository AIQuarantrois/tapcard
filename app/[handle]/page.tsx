...
export async function generateMetadata(
  ...
  const base = process.env.NEXT_PUBLIC_APP_URL ?? 'https://tapcard-v3ml.vercel.app'
  const ogImageUrl = `${base}/api/og?handle=${card.handle}`
...
export default async function PublicCardPage(
  ...
  // ── P0 FIX : utilise la fonction RPC sécurisée au lieu d'un UPDATE direct ──
  // L'ancien code faisait un UPDATE brut qui cassait avec la nouvelle RLS (owner-only).
  // increment_view_count() est une fonction SECURITY DEFINER côté Supabase :
  // elle s'exécute avec les droits de son propriétaire, jamais exposée en UPDATE public.
  supabase.rpc('increment_view_count', { card_handle: card.handle })
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0A0A0F', fontFamily:OT, position:'relative' }}>
      ...
      {/* ── Orbes décoratifs ── */}
      <div style={{ position:'fixed', top:-180, right:-180, width:520, height:520,
        borderRadius:'50%', pointerEvents:'none', zIndex:0,
        background:`radial-gradient(circle, ${grad.ac}1A, transparent 68%)` }}/>
      <div style={{ position:'fixed', bottom:-160, left:-160, width:440, height:440,
        borderRadius:'50%', pointerEvents:'none', zIndex:0,
      ...
      <div style={{ position:'relative', zIndex:1, maxWidth:480, margin:'0 auto', padding:'28px 16px 72px' }}>
        ...
        <div className="fu1" style={{ marginBottom:24 }}>
          <BusinessCard
            u={{ name:card.name, role:card.role, company:card.company,
                 linkedin:card.linkedin, socials:card.socials ?? {},
                 av, logo_url:card.logo_url, gradient:grad,
                 handle:card.handle, template:card.template, font:card.font }}
            large
          />
        </div>

        {/* CTA principal — conditionnel (AddContactButton gère l'état via localStorage) */}
            ...
            <div style={{ background:'#141418', borderRadius:14, overflow:'hidden',
              ...
              {(
                [
                  ...
                  card.website && { icon:<Globe  size={14} strokeWidth={1.5} color="rgba(245,245,247,0.38)"/>,
                    href: card.website.startsWith('http') ? card.website : `https://${card.website}`,
                    label: card.website.replace(/^https?:\/\//, '') },
                  card.address && { icon:<MapPin size={14} strokeWidth={1.5} color="rgba(245,245,247,0.38)"/>,
                    href:`https://maps.google.com/?q=${encodeURIComponent(card.address)}`,
                    label:card.address },
                ] as (false | { icon:React.ReactNode; href:string; label:string })[]
              ).filter(Boolean).map((r, i, arr) => {
                const row = r as { icon:React.ReactNode; href:string; label:string }
                return (
        ...
        {soc.length > 0 && (
          <div className="fu4" style={{ marginBottom:14 }}>
            ...
            <div style={{
              display:'grid',
              gridTemplateColumns:`repeat(${Math.min(soc.length, 4)}, 1fr)`,
              gap:8,
            }}>
              {soc.map(s => (
        ...
        <div className="fu5" style={{ marginBottom:14 }}>
          <div style={{ background:'#141418', borderRadius:14, padding:'20px 16px',
            ...
            <div style={{ position:'relative', padding:12, background:'#fff', borderRadius:16,
              ...
              <img
                src={`/api/qr?handle=${card.handle}`}
                width={180} height={180}
                alt={`QR code de ${card.name}`}
