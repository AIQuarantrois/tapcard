...
import {
  ...
  Globe, MapPin, Phone, Zap, Download,
} from 'lucide-react'
...
/* ══ SHARED UI — defined outside root to avoid recreation on render ══ */
...
function TextRow({ value, onChange, type='text', placeholder, last=false, prefix, theme, autoComplete, maxLength }: {
  value: string; onChange: (v: string) => void; type?: string; placeholder: string
  last?: boolean; prefix?: React.ReactNode; theme: Theme; autoComplete?: string; maxLength?: number
}) {
  ...
  return (
    <Row last={last} theme={theme}>
      <div style={{ display:'flex', alignItems:'center', minHeight:46, gap:10 }}>
        ...
        {nearLimit && maxLength && (
          <span style={{ fontSize:10, fontFamily:OT, flexShrink:0,
...
function Skeleton({ w='100%', h=20, r=6 }: { w?: string|number; h?: number; r?: number }) {
  return <div style={{ width:w, height:h, borderRadius:r,
    background:'rgba(255,255,255,0.07)', animation:'skPulse 1.4s ease-in-out infinite' }}/>
}

/* LogoMark — replace SVG content with <img src="/logo.svg"> once you have the asset */
function LogoMark({ size=32, grad }: { size?: number; grad: GradientState }) {
  return (
    <div style={{ width:size, height:size, borderRadius:Math.round(size*0.3), background:grad.css,
      display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0,
      boxShadow:`0 4px 16px ${grad.sh}` }}>
...
export default function TapCardApp() {
  ...
  /* ── Inject skeleton keyframe ── */
  useEffect(() => {
    const el = document.createElement('style')
    el.textContent = `@keyframes skPulse { 0%,100%{opacity:.4} 50%{opacity:.9} }`
    ...
    fetch(`/api/cards?handle=${handle}`)
      ...
      .then(data => {
        ...
        setUser({ name:data.name||'', role:data.role, company:data.company,
          ...
          view_count:data.view_count ?? 0, template:data.template ?? 'gradient', font:data.font ?? 'serif' })
        setScreen('mycard')
      })
    ...
    supabaseBrowser.auth.getSession().then(({ data: { session } }) => {
      ...
      fetch('/api/cards?handle=me', { headers: { Authorization: `Bearer ${session.access_token}` } })
        ...
        .then(data => {
          ...
          const av = ((p[0]?.[0] ?? '') + (p[1]?.[0] ?? '')).toUpperCase() || 'TC'
          const g  = data.gradient ? makeGrad(data.gradient.c1, data.gradient.c2, data.gradient.ac) : grad
          setGrad(g); setTemplate(data.template === 'solid' ? 'solid' : 'gradient')
          setFont(data.font === 'mono' ? 'mono' : data.font === 'serif' ? 'serif' : 'sans')
          setUser({ name:data.name||'', role:data.role, company:data.company,
            email:data.email, phone:data.phone, phone2:data.phone2,
            website:data.website, address:data.address, linkedin:data.linkedin,
  ...
  useEffect(() => {
    ...
    fetch(`/api/connections?handle=${user.handle}`)
      .then(r => r.ok ? r.json() : [])
      .then((data: Contact[]) => { setConnectionCount(data.length); setContacts(data); setContactsLoaded(true) })
  ...
  const doCreate = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cards', { method:'POST', headers:{ 'Content-Type':'application/json' },
        body: JSON.stringify({ handle:handleBase, name:form.name.trim(), role:form.role, company:form.company,
          ...
          gradient:{ c1:grad.c1, c2:grad.c2, ac:grad.ac }, logo_url:logoUrl, country_code:country.code, template, font }) })
      ...
      if (res.ok) {
        ...
        setConnectionCount(0); setOnboardStep(2)
  ...
  const doUpdate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setIsEditing(false); setOnboardStep(1); setScreen('mycard')
      } else {
  ...
  const startEditing = () => {
    if (!user) return
    const phoneNum = user.phone && user.country ? user.phone.replace(user.country.dial + ' ', '') : user.phone || ''
    ...
    setUpdateError(''); setIsEditing(true); setScreen('onboarding')
  }
  ...
  const doSendMagicLink = async () => {
    ...
    try {
      const { error } = await supabaseBrowser.auth.signInWithOtp({
        email: authEmail.trim(), options: { emailRedirectTo: window.location.origin } })
  ...
  const doSignOut = async () => {
    ...
    setOnboardStep(1); setScreen('landing')
  }
  const doNativeShare = async (handle: string, name: string) => {
    const url = `${window.location.origin}/${handle}`
    if (navigator.share) { try { await navigator.share({ title:name, text:'Ma carte TapCard', url }); return } catch {} }
    setShare(true)
  }
  ...
  /* ── Inline render helpers — called as functions to preserve focus ──
     IMPORTANT: These are called as {ThemeBtn()} not <ThemeBtn/> to prevent
     React from treating them as new components on every render, which would
     unmount/remount inputs and lose focus. */
          ...
          return (
            <button key={p.id} onClick={() => { setGrad(g); setShowCustom(false) }}
              style={{ flex:1, height:36, borderRadius:10, background:g.css,
                ...
                transition:'all .22s cubic-bezier(.34,1.56,.64,1)', transform: active ? 'scale(1.08)' : 'scale(1)',
                position:'relative' }}>
              {active && <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                <Check size={12} color="rgba(255,255,255,.9)" strokeWidth={3}/></div>}
            </button>
          )
        })}
      ...
      {showCustom && (
        <div className="exp" style={{ padding:'14px 16px 16px' }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'flex-end', marginBottom:12 }}>
            {([['Couleur 1', customC1, setCustomC1], ['Couleur 2', customC2, setCustomC2]] as const).map(([label, val, setter], idx) => (
              <div key={idx} style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:11, color:T.t3, fontFamily:OT, marginBottom:5 }}>{label}</div>
                <div style={{ display:'flex', alignItems:'center', gap:6, background:T.s2,
                  ...
                  <input type="text" value={val}
                    onChange={e => /^#[0-9A-Fa-f]{0,6}$/.test(e.target.value) && setter(e.target.value)}
          ...
          <div style={{ display:'flex', gap:10, alignItems:'center' }}>
            ...
            <button onClick={applyCustom} className="press" style={{
              ...
              color:'#fff', fontSize:13, fontFamily:OT, fontWeight:600 }}>Appliquer</button>
          </div>
        </div>
      )}
    </div>
  ...
  const TemplatePicker = () => {
    const solidBg = grad.c1; const ratio = contrastRatio(autoText(solidBg), solidBg)
    const ok = ratio >= 4.5; const mid = ratio >= 3; const isDk = lum(solidBg) <= 0.22
    return (
            ...
            return (
              <button key={t.id} onClick={() => setTemplate(t.id)}
                style={{ flex:1, borderRadius:8, overflow:'hidden', padding:0,
                  border:`2px solid ${active ? grad.ac : T.sep}`, transition:'all .18s', background:T.s2, cursor:'pointer' }}>
                {t.id === 'gradient' && (
                  <div style={{ height:52, background:grad.css, display:'flex', flexDirection:'column',
                    ...
                    <div><div style={{ height:5, borderRadius:2, marginBottom:3, background:'rgba(255,255,255,.85)', width:36 }}/><div style={{ height:3, borderRadius:2, background:'rgba(255,255,255,.40)', width:24 }}/></div>
                  </div>
                )}
                {t.id === 'solid' && (
                  <div style={{ height:52, background:solidBg, display:'flex', flexDirection:'column',
                    alignItems:'flex-start', justifyContent:'space-between', padding:'8px 10px', borderLeft:`2.5px solid ${grad.ac}` }}>
                    <div style={{ width:16, height:16, borderRadius:4, background: isDk ? 'rgba(255,255,255,.14)' : 'rgba(0,0,0,.09)' }}/>
                    <div><div style={{ height:5, borderRadius:2, marginBottom:3, background: isDk ? 'rgba(255,255,255,.82)' : 'rgba(0,0,0,.72)', width:36 }}/><div style={{ height:3, borderRadius:2, background: isDk ? 'rgba(255,255,255,.35)' : 'rgba(0,0,0,.30)', width:24 }}/></div>
                  </div>
                )}
        ...
        {FONTS.map(f => {
          ...
          return (
            <button key={f.id} onClick={() => setFont(f.id)}
              style={{ flex:1, borderRadius:10, padding:'10px 8px', cursor:'pointer',
  ...
  /* ── Logo upload row — called as {LogoRow()} to preserve input focus ── */
  const LogoRow = () => (
    <Row theme={T}>
      <div style={{ display:'flex', alignItems:'center', minHeight:66, gap:16 }}>
        ...
        <button onClick={() => fileRef.current?.click()} style={{
          ...
          border:`2px dashed ${logoUrl ? 'transparent' : T.sepS}`,
          overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center',
          transition:'all .18s' }}>
          {logoUrl
            ? <img src={logoUrl} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }}/>
            : <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={T.t3} strokeWidth="1.5">
                <rect x="3" y="3" width="18" height="18" rx="3"/>
                <circle cx="8.5" cy="8.5" r="1.5" fill={T.t3} stroke="none"/>
                <path d="M21 15l-5-5L5 21"/>
              </svg>}
        </button>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:15, color:T.t1, fontWeight:500, marginBottom:2 }}>
            {logoUrl ? 'Logo enregistré' : 'Logo ou photo'}
          </div>
          <div style={{ fontSize:12, color:T.t3, lineHeight:1.5 }}>
            {logoUrl
              ...
              : <>PNG, JPG · carré recommandé<br/>
                  <span style={{ color:T.t4 }}>Modifiable à tout moment dans votre profil</span></>
            }
          </div>
        </div>
    ...
    <Section label="Lien public"
      ...
      <Row last theme={T}>
        <div style={{ display:'flex', alignItems:'center', minHeight:46, gap:8 }}>
          ...
          {handleStatus === 'ok'       && <span style={{ fontSize:12, color:'#22c55e', fontWeight:500 }}>✓ Disponible</span>}
      ...
      {showWarning && form.handle && form.handle !== user?.handle && handleStatus === 'ok' && (
        <div style={{ padding:'10px 16px', fontSize:11, color:'rgba(251,191,36,0.85)',
          borderTop:`1px solid ${T.sep}`, lineHeight:1.6 }}>
      ...
      {showMoreSoc && SOCIALS.map((s, i) => (
        <Row key={s.id} last={i===SOCIALS.length-1} theme={T}>
          <div style={{ display:'flex', alignItems:'center', minHeight:46, gap:12 }}>
            ...
            <input type="text" placeholder={s.ph} value={socials[s.id]||''}
              onChange={e => setSoc(s.id, e.target.value)}
              style={{ flex:1, background:'transparent', border:'none', outline:'none', color:T.t1, fontSize:15, fontFamily:OT }}/>
          </div>
        </Row>
      ))}
        ...
        <Row last onTap={() => setShowMoreSoc(true)} theme={T}>
          <div style={{ display:'flex', alignItems:'center', minHeight:46, gap:10 }}>
            ...
            <span style={{ fontSize:15, color:T.blue, fontWeight:500 }}>
              Autres réseaux
              {[form.linkedin,...Object.values(socials)].filter(v=>v?.trim()).length > 0 && (
                <span style={{ background:grad.css, borderRadius:10, padding:'1px 7px',
                  fontSize:10, fontWeight:600, color:'#fff', marginLeft:8, display:'inline-block', lineHeight:1.6 }}>
                  {[form.linkedin,...Object.values(socials)].filter(v=>v?.trim()).length}
                </span>
              )}
            </span>
          </div>
  ...
  /* ══════════════════════════════════════════════════════════ LANDING */
  if (screen === 'landing') {
    const demoUser = { name:'Sophie Martin', role:'Directrice Marketing', company:'Nexora',
      av:'SM', logo:null, gradient:grad, handle:'sophiemartin', socials:{ linkedin:'1', twitter:'1' }, linkedin:'1' }
    return (
      <div style={{ minHeight:'100vh', background:T.bg, fontFamily:OT,
        ...
        {pwaPrompt && !pwaInstalled && (
          <div style={{ position:'fixed', bottom:0, left:0, right:0, zIndex:100,
            background:T.s1, borderTop:`1px solid ${T.sep}`, padding:'14px 20px',
            display:'flex', alignItems:'center', gap:12 }}>
            <LogoMark size={36} grad={grad}/>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:14, color:T.t1, fontWeight:500 }}>Installer TapCard</div>
              <div style={{ fontSize:12, color:T.t3 }}>Accès rapide depuis votre écran d&apos;accueil</div>
            </div>
            <button onClick={doInstallPwa} style={{
              background:grad.css, borderRadius:9, padding:'9px 16px',
              color:'#fff', fontSize:13, fontFamily:OT, fontWeight:600,
              boxShadow:`0 4px 14px ${grad.sh}` }}>
              Installer
            </button>
            <button onClick={() => setPwaPrompt(null)} style={{ color:T.t3, padding:'4px' }}>
              <X size={16}/>
            </button>
          </div>
        )}
        ...
        <div style={{ display:'flex', minHeight:'100vh', position:'relative', zIndex:1 }}>
          {/* ── Left column ── */}
          <div style={{
            width: isDesktop ? 480 : '100%', flexShrink:0,
            /* FIX: use flex-start + paddingTop instead of justify-center to prevent content going off-screen on mobile */
            display:'flex', flexDirection:'column', justifyContent:'flex-start',
            padding: isDesktop ? '72px 48px 60px' : '52px 24px 32px',
            ...
            {/* Headline — always visible, even on small screens */}
            <div className="fu1" style={{ marginBottom:14 }}>
              <div style={{ fontFamily:CG, fontSize:isDesktop?44:34, fontWeight:600, color:T.t1,
            ...
            <div className="fu3" style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:32 }}>
              {[
                { icon:<Zap size={14}/>,    text:'QR code, lien direct ou NFC' },
                { icon:<Globe size={14}/>,  text:'Zéro installation pour le destinataire' },
                { icon:<Check size={14}/>,  text:'Mise à jour instantanée chez vos contacts' },
              ].map((v, i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <span style={{ fontSize:13, color:T.t2, fontWeight:300 }}>{v.text}</span>
                </div>
              ))}
            </div>
            ...
            <div className="fu4">
              ...
              <button onClick={() => { setAuthSent(false); setScreen('auth') }} style={{
                width:'100%', padding:'14px', borderRadius:14, background:T.t5,
                border:`1px solid ${T.sep}`, color:T.t2, fontSize:14, fontFamily:OT }}>
                J&apos;ai déjà une carte →
              </button>
            </div>

            <div style={{ marginTop:16, fontSize:11, fontWeight:300, color:T.t4, letterSpacing:.3 }}>
              Aucune inscription · RGPD · Données sécurisées
            </div>

            {/* PWA install link (fallback if banner not shown) */}
            {pwaPrompt && !pwaInstalled && (
              <button onClick={doInstallPwa} style={{ marginTop:16, display:'flex', alignItems:'center', gap:6,
                color:T.t3, fontSize:12, fontFamily:OT, background:'none', border:'none', cursor:'pointer', padding:0 }}>
          ...
          {isDesktop && (
            <div style={{ flex:1, display:'flex', flexDirection:'column',
              alignItems:'center', justifyContent:'center', padding:'60px 48px', gap:28 }}>
        ...
        {/* Mobile card preview */}
        {!isDesktop && (
          <div style={{ position:'relative', zIndex:1, padding:'8px 24px 64px' }}>
            <BusinessCard u={demoUser as any} large/>
            <div style={{ marginTop:10, fontSize:11, color:T.t4, fontWeight:300, textAlign:'center' }}>
              Aperçu · personnalisable en quelques secondes
            </div>
          </div>
        )}
  ...
  if (screen === 'onboarding') {
    ...
    const step1Done = form.name.trim().length > 0
    const step2Done = !!(form.email.trim() || form.linkedin.trim())

    /* ── FormContent — called as function {FormContent()} NOT <FormContent/> ──
       This is critical: calling as a JSX component causes React to unmount/remount
       on every parent re-render, losing input focus on each keystroke. */
    const FormContent = () => (
      <div style={{ padding:`0 ${isDesktop?40:16}px 130px` }}>

        {/* Sticky card preview — mobile only */}
        {!isDesktop && (
        ...
        {/* ══ STEP 1: Color first → then Identity ══ */}
        ...
        {!isEditing && onboardStep === 2 && (
          <>
            ...
            <div className="fu3">{SocialsSection()}</div>
            <div className="fu4">
              {!showMoreInfo ? (
                <button onClick={() => setShowMoreInfo(true)} style={{
                  width:'100%', padding:'12px 16px', borderRadius:12,
                  background:T.s1, border:`1px solid ${T.sep}`, color:T.t3,
                  fontSize:13, fontFamily:OT, marginBottom:24,
            ...
            <div className="fu5">{HandleSection({})}</div>
            <div className="fu6">
              {updateError && (
              ...
              <button onClick={doUpdate}
                ...
                className="press" style={{
                  width:'100%', padding:'16px', borderRadius:14,
                  background:handleStatus !== 'taken' ? grad.css : T.s2, color:'#fff',
                  fontSize:16, fontWeight:600, fontFamily:OT,
              ...
              <div style={{ textAlign:'center', marginTop:10, fontSize:11, color:T.t4, lineHeight:1.6 }}>
                Toutes ces informations sont modifiables à tout moment.
              </div>
            </div>
          </>
        )}
        ...
        {isEditing && (
          <>
            ...
            <div className="fu6">
              {HandleSection({ showWarning: true })}
              {SocialsSection()}
              {updateError && (
                <div style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.3)',
                  borderRadius:12, padding:'12px 16px', marginBottom:12, fontSize:13, color:T.red, lineHeight:1.6 }}>
              ...
              <button onClick={doUpdate}
                ...
                className="press" style={{
                  width:'100%', padding:'16px', borderRadius:14,
                  background:form.name.trim() && handleStatus !== 'taken' ? grad.css : T.s2, color:'#fff',
    ...
    return (
      <div style={{ minHeight:'100vh', background:T.bg, fontFamily:OT, transition:'background .3s', position:'relative' }}>
        ...
        {isDesktop ? (
          <div style={{ display:'flex', minHeight:'100vh', position:'relative', zIndex:1 }}>
            <div style={{ width:480, flexShrink:0, overflowY:'auto', height:'100vh', borderRight:`1px solid ${T.sep}` }}>
              <div style={{ padding:'40px 40px 0', display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
                <div>
                  <div style={{ fontFamily:CG, fontSize:26, fontWeight:600, color:T.t1, letterSpacing:-.4, lineHeight:1 }}>
                  ...
                  <div style={{ fontFamily:OT, fontSize:12, fontWeight:300, color:T.t3, marginTop:4 }}>
                    {isEditing ? 'Vos informations' : onboardStep === 1 ? 'Couleur · nom · poste' : 'Contact · réseaux · lien'}
                  </div>
                </div>
              ...
              {!isEditing && (
                <div style={{ padding:'0 40px', display:'flex', gap:5, marginBottom:16 }}>
                  {[step1Done, step2Done].map((ok, i) => (
                    <div key={i} style={{ flex:1, height:3, borderRadius:2, transition:'background .4s',
                      background: ok ? grad.ac : (i < (onboardStep-1) ? T.sepS : T.sep) }}/>
                  ))}
                </div>
              )}
              {/* Called as function — preserves focus across re-renders */}
              {FormContent()}
            </div>
            <div style={{ flex:1, display:'flex', flexDirection:'column',
          ...
          <div style={{ position:'relative', zIndex:1 }}>
            <div style={{ padding:'48px 20px 0', display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
              <div>
                <div style={{ fontFamily:CG, fontSize:24, fontWeight:600, color:T.t1, letterSpacing:-.4, lineHeight:1 }}>
                  {isEditing ? 'Modifier ma carte' : onboardStep === 1 ? 'Créer ma carte' : 'Finaliser'}
                </div>
                <div style={{ fontFamily:OT, fontSize:12, fontWeight:300, color:T.t3, marginTop:3 }}>
                  {isEditing ? 'Vos informations' : onboardStep === 1 ? 'Couleur · nom · poste' : 'Contact · réseaux · lien'}
                </div>
              </div>
              <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            ...
            {!isEditing && (
              <div style={{ padding:'0 20px', display:'flex', gap:5, marginBottom:18 }}>
                {[step1Done, step2Done].map((ok, i) => (
                  <div key={i} style={{ flex:1, height:3, borderRadius:2, transition:'background .4s',
                    background: ok ? grad.ac : (i < (onboardStep-1) ? T.sepS : T.sep) }}/>
                ))}
            ...
            {/* Called as function — critical for focus preservation on mobile */}
            {FormContent()}
          </div>
        )}
        {CountrySheet()}
            ...
            <div style={{ background:T.s1, borderRadius:14, overflow:'hidden', border:`1px solid ${T.sep}`, marginBottom:14 }}>
              <div style={{ padding:'0 16px', display:'flex', alignItems:'center', minHeight:54, gap:12 }}>
                ...
                <input type="email" value={authEmail} onChange={e => setAuthEmail(e.target.value)}
                  ...
                  style={{ flex:1, background:'transparent', border:'none', outline:'none',
                    color:T.t1, fontSize:16, fontFamily:OT }}/>
              </div>
            </div>
            <button onClick={doSendMagicLink} disabled={!authEmail.trim() || authSending}
              className="press" style={{
                width:'100%', padding:'16px', borderRadius:14,
                background:authEmail.trim() ? grad.css : T.s2, color:'#fff',
                fontSize:16, fontWeight:600, fontFamily:OT,
  ...
  /* ══════════════════════════════════════════════════════════ MY CARD */
  if (screen === 'mycard') {
    const u = user ?? { name:'Alex Dupont', role:'CEO & Co-Founder', company:'Nexora Labs',
      email:'alex@nexora.io', phone:'+33 6 12 34 56 78',
      linkedin:'linkedin.com/in/alexdupont', socials:{ twitter:'@alexdupont', github:'github.com/alexdupont' },
    ...
    return (
      <div style={{ minHeight:'100vh', background:T.bg, fontFamily:OT, position:'relative', transition:'background .3s' }}>
        ...
        {/* Desktop right panel */}
        {isDesktop && (
          <div style={{ position:'fixed', top:0, bottom:0, left:480, right:0, zIndex:1,
            display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
            padding:'60px 48px', gap:28, borderLeft:`1px solid ${T.sep}`, background:T.bg }}>
            <div style={{ width:'100%', maxWidth:380 }}>
            ...
            <div style={{ display:'flex', gap:16, alignItems:'center' }}>
              ...
              <div>
                <div style={{ fontFamily:CG, fontSize:15, fontWeight:600, color:T.t1, marginBottom:3 }}>
                  tapcard.io/{u.handle}
                </div>
        ...
        <div style={{ maxWidth:480, position:'relative', zIndex:1,
          ...
          <div style={{ padding:'52px 20px 0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <LogoMark size={34} grad={g}/>
            <div style={{ display:'flex', alignItems:'center', gap: isDesktop ? 4 : 10 }}>
              {isDesktop && navItems.map(item => (
                <button key={item.id} onClick={() => setNav(item.id)} style={{
                  ...
                  background: nav === item.id ? `${g.ac}18` : 'transparent',
                  border:`1px solid ${nav === item.id ? `${g.ac}40` : 'transparent'}`,
                  color: nav === item.id ? g.ac : T.t2,
                  fontSize:13, fontFamily:OT, fontWeight: nav === item.id ? 600 : 400, transition:'all .18s' }}>
                  {item.ic} {item.l}
                </button>
              ))}
              {ThemeBtn()}
            ...
            <div style={{ padding:'24px 16px' }}>
              <div className="fu1" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:22 }}>
                {[
                  ...
                  { l:'Contacts', v: connectionCount !== null ? String(connectionCount) : null },
                  ...
                  <div key={s.l} style={{ background:T.s1, border:`1px solid ${T.sep}`,
                    ...
                    {s.v !== null
                      ...
                      : <div style={{ display:'flex', justifyContent:'center', marginBottom:4 }}><Skeleton w={36} h={24} r={4}/></div>
                    }
              ...
              {/* PWA install inline prompt */}
              {pwaPrompt && !pwaInstalled && (
                <div style={{ marginTop:16, display:'flex', alignItems:'center', gap:10,
                  background:T.s1, border:`1px solid ${T.sep}`, borderRadius:12, padding:'12px 14px' }}>
                  <Download size={15} color={g.ac}/>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, color:T.t1, fontWeight:500 }}>Installer l&apos;app</div>
                    <div style={{ fontSize:11, color:T.t3 }}>Accès depuis votre écran d&apos;accueil</div>
                  </div>
                  <button onClick={doInstallPwa} style={{
                    background:g.css, borderRadius:8, padding:'7px 14px',
                    color:'#fff', fontSize:12, fontFamily:OT, fontWeight:600,
                    boxShadow:`0 3px 10px ${g.sh}` }}>
                    Installer
                  </button>
                </div>
              )}
          ...
          {nav === 'contacts' && (() => {
            ...
            const filtered = contacts
              .filter(c => { if (!q) return true
                const n=(c.card?.name??c.contact_handle).toLowerCase(); const r=(c.card?.role??'').toLowerCase(); const co=(c.card?.company??'').toLowerCase()
                return n.includes(q)||r.includes(q)||co.includes(q) })
            ...
            const PAGE=20; const visible=filtered.slice(0,contactPage*PAGE); const hasMore=filtered.length>visible.length
            return (
              <div style={{ padding:'24px 16px' }}>
                ...
                {contactsLoaded && contacts.length > 0 && (
                  <div className="fu2" style={{ marginBottom:16, display:'flex', gap:8 }}>
                    <div style={{ flex:1, display:'flex', alignItems:'center', gap:8,
                      ...
                      {contactSearch && <button onClick={() => setContactSearch('')} style={{ color:T.t3, display:'flex', alignItems:'center' }}><X size={13}/></button>}
                    </div>
                      ...
                      {(['date','name'] as const).map(s => (
                        <button key={s} onClick={() => setContactSort(s)}
                          style={{ padding:'10px 12px', fontSize:12, fontFamily:OT, fontWeight:500,
                            background: contactSort===s ? T.s3 : 'transparent', color: contactSort===s ? T.t1 : T.t3,
                    ...
                    {[1,2,3].map(i => (
                      <div key={i} style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 0' }}>
                        ...
                        <div style={{ flex:1, display:'flex', flexDirection:'column', gap:6 }}>
                          <Skeleton w="60%" h={14} r={4}/><Skeleton w="40%" h={11} r={3}/>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {contactsLoaded && contacts.length===0 && (
                  <div style={{ textAlign:'center', padding:'40px 20px' }}>
                    ...
                    <div style={{ fontSize:13, color:T.t3, lineHeight:1.7, fontWeight:300 }}>Partagez votre carte pour recevoir des connexions en retour.</div>
                  </div>
                )}
                {contactsLoaded && contacts.length>0 && filtered.length===0 && (
                  <div style={{ textAlign:'center', padding:'32px 20px', color:T.t3, fontSize:14 }}>Aucun résultat pour « {contactSearch} »</div>
                )}
                {contactsLoaded && visible.length>0 && (
                  <>
                    <Section theme={T}>
                      {visible.map((c,i,arr) => {
                        const name=c.card?.name??c.contact_handle; const parts=name.split(' ')
                              ...
                              <div style={{ flex:1, minWidth:0 }}>
                                ...
                                {c.met_location && (
                                  <div style={{ display:'flex', alignItems:'center', gap:4, marginTop:3 }}>
                                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={T.t4} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                      <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/><circle cx="12" cy="9" r="2.5"/>
                                    </svg>
                                    <span style={{ fontSize:11, color:T.t4, fontWeight:300, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{c.met_location}</span>
                                  </div>
                                )}
                              </div>
                ...
                {authUser ? (
                  <>
                    ...
                    <Row theme={T} onTap={startEditing}>
                      <div style={{ display:'flex', alignItems:'center', minHeight:50, justifyContent:'space-between' }}>
                        <div><div style={{ fontSize:15, color:T.t1 }}>Modifier ma carte</div><div style={{ fontSize:12, color:T.t3, marginTop:1, fontWeight:300 }}>Nom, poste, couleur, logo</div></div>
                        <ChevronRight size={15} color={T.t4}/>
                      </div>
                    </Row>
                    <Row theme={T} onTap={() => window.open(`/${u.handle}`,'_blank')}>
                      <div style={{ display:'flex', alignItems:'center', minHeight:50, justifyContent:'space-between' }}>
                        <div><div style={{ fontSize:15, color:T.t1 }}>Lien personnalisé</div><div style={{ fontSize:12, color:T.t3, marginTop:1, fontWeight:300 }}>tapcard.io/{u.handle}</div></div>
                        <ChevronRight size={15} color={T.t4}/>
                      </div>
                    </Row>
                  ...
                  <>
                    ...
                    <Row last onTap={() => { setAuthSent(false); setScreen('auth') }} theme={T}>
                      <div style={{ display:'flex', alignItems:'center', minHeight:50, justifyContent:'space-between' }}>
                        <div><div style={{ fontSize:15, color:T.t1 }}>Sécuriser ma carte</div><div style={{ fontSize:12, color:T.t3, marginTop:1, fontWeight:300 }}>Accès multi-appareils par email</div></div>
                        <ChevronRight size={15} color={T.t4}/>
                      </div>
                    </Row>
                  </>
                    ...
                    <div style={{ display:'flex', alignItems:'center', minHeight:50, justifyContent:'space-between' }}>
                      <div>
                        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                          ...
                          {it.badge && <span style={{ background:g.css, borderRadius:5, padding:'1px 7px', fontSize:9, fontWeight:600, color:'#fff', letterSpacing:.8 }}>{it.badge}</span>}
                        </div>
            ...
            <div style={{ display:'flex', justifyContent:'space-around' }}>
              {navItems.map(item => (
                <button key={item.id} onClick={() => setNav(item.id)} style={{
                  ...
                  color:nav===item.id ? g.ac : T.t3, fontSize:10, fontFamily:OT,
                  fontWeight:nav===item.id ? 500 : 400, transition:'color .16s', flex:1 }}>
        ...
        {share && (
          <div className="fi" style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.55)', zIndex:100 }}>
            ...
            <div className="su" style={{ position:'absolute', bottom:0,
              left:isDesktop?'auto':0, right:0, width:isDesktop?480:'100%',
                ...
                {stab === 'qr' && (
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:22, width:'100%' }}>
                    ...
                    <div style={{ textAlign:'center' }}>
                      <div style={{ fontFamily:CG, fontSize:18, fontWeight:600, color:T.t1, marginBottom:6 }}>tapcard.io/{u.handle}</div>
                ...
                {stab === 'nfc' && (
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:22, width:'100%', paddingTop:14 }}>
                    <div style={{ position:'relative', width:116, height:116, borderRadius:'50%',
                      background:T.s2, border:`1px solid ${T.sep}`, display:'flex', alignItems:'center', justifyContent:'center' }}>
                      <div style={{ position:'absolute', inset:-14, borderRadius:'50%', background:g.sh.replace('80','25'), animation:'pulse 2.4s ease-out infinite' }}/>
                    ...
                    <button className="press" style={{ background:g.css, borderRadius:12, padding:'14px', color:'#fff',
                      fontSize:14, fontWeight:600, fontFamily:OT, boxShadow:`0 8px 28px ${g.sh}`, width:'100%', letterSpacing:.2 }}>
        ...
        {showScanner && (
          <Suspense fallback={null}>
            <QRScanner gradCss={g.css} onClose={() => setShowScanner(false)}
              onResult={async scannedHandle => {
                ...
                if (user) {
                  try {
                    ...
                    setConnectionCount(c => (c??0)+1); setContactsLoaded(false)
                  } catch {}
                }
              }}/>
          </Suspense>
