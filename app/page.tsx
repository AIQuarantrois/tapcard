...
const MAX_W = 480          // max content width px
...
/* ══ SHARED COMPONENTS — defined outside root for stable identity ══ */
...
function TRow({ value, onChange, type='text', placeholder, last=false, prefix, T, autoComplete, maxLength }: {
  ...
  const near = maxLength !== undefined && value.length >= Math.floor(maxLength * 0.8)
...
/* LogoMark — swap SVG with <img src="/logo.svg"> when asset is ready */
...
export default function TapCardApp() {
  ...
  /* bg with transition */
  const bgStyle = { background: T.bg, transition:'background .25s, color .25s' } as const
    ...
    fetch(`/api/cards?handle=${handle}`)
      ...
      .then(d => {
        ...
        setUser({name:d.name,role:d.role,company:d.company,email:d.email,phone:d.phone,phone2:d.phone2,
          ...
          socials:d.socials||{},av,logo:d.logo_url,gradient:g,
  ...
  useEffect(() => {
    ...
    fetch(`/api/connections?handle=${user.handle}`)
      .then(r=>r.ok?r.json():[])
      .then(d=>{setContacts(d);setCtLoaded(true)})
    ...
    try {
      ...
      if(res.ok){
        setUser({name:form.name.trim(),role:form.role,company:form.company,
          ...
          linkedin:form.linkedin,handle:d.handle,socials,av,logo:logoUrl,gradient:grad,country,
        ...
        setCtCount(0); setStep(2); window.scrollTo({top:0,behavior:'instant'})
      }
    ...
    try {
      ...
      if(res.ok){
        ...
        setUser({name:form.name.trim(),role:form.role,company:form.company,
          email:form.email,phone:form.phone?`${country.dial} ${form.phone}`:'',
          phone2:form.phone2,website:form.website,address:form.address,
          linkedin:form.linkedin,handle:fh,socials,av,logo:logoUrl,gradient:grad,country,view_count:user.view_count,template,font})
  ...
  const startEdit = () => {
    ...
    setLogoUrl(user.logo||null); setUpdErr(''); setIsEditing(true); setScreen('onboarding')
  }
  ...
  const doMagicLink = async () => {
    ...
    try {
      const {error}=await supabaseBrowser.auth.signInWithOtp({email:authEmail.trim(),options:{emailRedirectTo:window.location.origin}})
  ...
  /* ══════════════════════════════════════════════════════════
     FIXED HEADER — universal across all screens
     Logo left · Nav center (desktop mycard) · Actions right
  ══════════════════════════════════════════════════════════ */
  const AppHeader = ({
    title, subtitle, back, actions, navItems, currentNav, onNav, borderless
  }: {
    title?: React.ReactNode
    subtitle?: string
    back?: ()=>void
    actions?: React.ReactNode
    navItems?: {id:Nav;l:string;ic:React.ReactNode}[]
    currentNav?: Nav
    onNav?: (n:Nav)=>void
    borderless?: boolean
  }) => {
    ...
    return (
      <div style={{
        position:'fixed', top:0, left:0, right:0, zIndex:80, height:HDR_H,
        background: dark ? 'rgba(10,10,15,.88)' : 'rgba(242,242,247,.88)',
        backdropFilter:'blur(24px) saturate(1.8)',
        borderBottom: borderless ? 'none' : `1px solid ${T.sep}`,
        display:'flex', alignItems:'center',
        padding:`0 ${isDesktop?'20px':'16px'}`, gap:10,
        transition:'background .25s',
      }}>
        {/* Back button */}
        {back && (
          <button onClick={back} style={{ width:32, height:32, borderRadius:9,
        ...
        {/* Logo mark — always top-left */}
        {!back && <LogoMark size={28} grad={user?.gradient ?? grad}/>}

        {/* Title / subtitle */}
        {(title || subtitle) && (
          <div style={{ flex: navItems ? 0 : 1 }}>
            {title && <div style={{ fontFamily:CG, fontSize:18, fontWeight:600, color:T.t1,
              letterSpacing:-.3, lineHeight:1 }}>{title}</div>}
            {subtitle && <div style={{ fontFamily:OT, fontSize:11, color:T.t3, marginTop:2, fontWeight:300 }}>{subtitle}</div>}
          </div>
        )}

        {/* Desktop inline nav (mycard) */}
        {navItems && onNav && (
        ...
        {!navItems && <div style={{ flex:1 }}/>}

        {/* Right actions: custom + theme + PWA */}
        <div style={{ display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
          {actions}
          {/* PWA install icon */}
          {pwaPrompt && !pwaInstalled && (
            <button onClick={doInstall} title="Installer l'application"
              style={{ width:34, height:34, borderRadius:10, background:T.t5, border:`1px solid ${T.sep}`,
  ...
  const Orbs = () => (
    <>
      <div style={{ position:'fixed', top:-160, right:-160, width:480, height:480, borderRadius:'50%',
        pointerEvents:'none', zIndex:0,
        background:`radial-gradient(circle,${grad.ac}18,transparent 68%)`, filter:'blur(60px)' }}/>
      <div style={{ position:'fixed', bottom:-140, left:-140, width:400, height:400, borderRadius:'50%',
        pointerEvents:'none', zIndex:0,
        ...
        <div style={{display:'flex',gap:8,padding:'14px 16px 10px'}}>
          {TEMPLATES.map(t=>{const active=template===t.id; return (
            <button key={t.id} onClick={()=>setTemplate(t.id)}
              style={{flex:1,borderRadius:8,overflow:'hidden',padding:0,border:`2px solid ${active?grad.ac:T.sep}`,transition:'all .18s',background:T.s2,cursor:'pointer'}}>
  ...
  /* ── Logo upload row — function call preserves focus ── */
  const LogoRow = () => (
    <Row T={T}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:12,color:T.t3,lineHeight:1.5}}>
            {logoUrl
              ...
              : <>PNG, JPG · carré recommandé<br/><span style={{color:T.t4}}>Modifiable à tout moment dans Profil</span></>}
          </div>
        </div>
    ...
    <Section label="Lien public" footer={`tapcard.io/${form.handle||(form.name.split(' ')[0]||'vous').toLowerCase()}`} T={T}>
      <Row last T={T}>
        <div style={{display:'flex',alignItems:'center',minHeight:46,gap:8}}>
          ...
          <input type="text" placeholder={(form.name.split(' ')[0]||'vous').toLowerCase()}
  ...
  const SocialsSection=()=>(
    <Section label="Réseaux sociaux" T={T}>
      ...
      {showMoreSoc&&SOCIALS.map((s,i)=>(
        <Row key={s.id} last={i===SOCIALS.length-1} T={T}>
          <div style={{display:'flex',alignItems:'center',minHeight:46,gap:12}}>
          ...
          <div style={{display:'flex',alignItems:'center',minHeight:46,gap:10}}>
            ...
            <span style={{fontSize:15,color:T.blue,fontWeight:500}}>
              ...
              {[form.linkedin,...Object.values(socials)].filter(v=>v?.trim()).length>0&&(
                <span style={{background:grad.css,borderRadius:10,padding:'1px 7px',fontSize:10,fontWeight:600,color:'#fff',marginLeft:8,display:'inline-block',lineHeight:1.6}}>
  ...
  /* ══ SPLASH ══ */
  if(screen==='splash') return (
    <div style={{minHeight:'100vh',...bgStyle,display:'flex',alignItems:'center',justifyContent:'center'}}>
  ...
  if(screen==='landing'){
    ...
    return (
      <div style={{minHeight:'100vh',...bgStyle,fontFamily:OT,position:'relative',overflow:'hidden'}}>
        <Orbs/>
        {AppHeader({
          borderless: true,
          actions: (
            <button onClick={()=>{setAuthSent(false);setScreen('auth')}}
              style={{background:T.t5,border:`1px solid ${T.sep}`,borderRadius:10,
                padding:'6px 14px',color:T.t2,fontSize:13,fontFamily:OT}}>
              Se connecter
            </button>
          )
        })}
        ...
        <div style={{display:'flex',minHeight:'100vh',position:'relative',zIndex:1}}>
          {/* Left */}
          <div style={{
            width:isDesktop?MAX_W:'100%', flexShrink:0,
            display:'flex', flexDirection:'column', justifyContent:'flex-start',
            padding:isDesktop?`${HDR_H+48}px 48px 60px`:`${HDR_H+40}px 24px 32px`,
            ...
            <div className="fu3" style={{display:'flex',flexDirection:'column',gap:9,marginBottom:32}}>
              {[
                {ic:<Zap size={14}/>,t:'QR code, lien direct ou NFC'},
                {ic:<Globe size={14}/>,t:'Zéro installation pour le destinataire'},
                {ic:<Check size={14}/>,t:'Mise à jour en temps réel chez vos contacts'},
              ].map((v,i)=>(
                <div key={i} style={{display:'flex',alignItems:'center',gap:10}}>
                  <div style={{width:26,height:26,borderRadius:7,flexShrink:0,
                    background:`${grad.ac}20`,border:`1px solid ${grad.ac}30`,
                    display:'flex',alignItems:'center',justifyContent:'center',color:grad.ac}}>{v.ic}</div>
            ...
            <div className="fu4">
              ...
              <button onClick={()=>{setAuthSent(false);setScreen('auth')}} style={{
                ...
                background:T.t5,border:`1px solid ${T.sep}`,color:T.t2,fontSize:14,fontFamily:OT}}>
                J&apos;ai déjà une carte →
              </button>
            </div>
            <div style={{marginTop:14,fontSize:11,fontWeight:300,color:T.t4,letterSpacing:.3}}>
          ...
          {/* Right desktop */}
          {isDesktop&&(
            <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'60px 48px',gap:28,paddingTop:`${HDR_H+60}px`}}>
              <div style={{width:'100%',maxWidth:380}}>
                <BusinessCard u={demo as any} large floating/>
              </div>
  ...
  if(screen==='onboarding'){
    const pn=form.name||'Votre Nom'; const pr=form.role||'Poste'
    const pav=((pn.split(' ')[0]?.[0]??'V')+(pn.split(' ')[1]?.[0]??'N')).toUpperCase()
    const ph=form.handle||(pn.split(' ')[0]||'vous').toLowerCase()
    const pu={name:pn,role:pr,company:form.company,socials,linkedin:form.linkedin,
              av:pav,logo:logoUrl,gradient:grad,handle:ph,template,font}
    const s1ok=form.name.trim().length>0
    const s2ok=!!(form.email.trim()||form.linkedin.trim())

    /* ── FormContent called as function ── */
    const FormContent=()=>(
      <div style={{padding:`0 ${isDesktop?40:16}px 130px`}}>
        ...
        {/* ══ STEP 1: Color → Logo/Company → Name → Role ══ */}
        {!isEditing&&step===1&&(
          <>
            {ColorPicker()}
            <div className="fu2">
              <Section label="Identité" T={T}>
                {/* Logo upload — prominent, at the top of identity */}
                {LogoRow()}
                <TRow placeholder="Prénom et Nom" value={form.name}
                  onChange={v=>setForm(p=>({...p,name:v}))} T={T} autoComplete="name" maxLength={40}/>
                <TRow placeholder="Poste / Titre" value={form.role}
                  onChange={v=>setForm(p=>({...p,role:v}))} T={T} autoComplete="organization-title" maxLength={50}/>
                <TRow placeholder="Entreprise" value={form.company}
                  onChange={v=>setForm(p=>({...p,company:v}))} last T={T} autoComplete="organization" maxLength={60}/>
              </Section>
            </div>
        ...
        {/* ══ STEP 2: Contact + Socials + Handle ══ */}
        {!isEditing&&step===2&&(
          <>
            <div className="fu1">
              <Section label="Contact" T={T}>
                <TRow type="email" placeholder="Email professionnel" value={form.email}
                  onChange={v=>setForm(p=>({...p,email:v}))} prefix={<Mail size={15} strokeWidth={1.5}/>} T={T} autoComplete="email"/>
                <Row T={T} last>
            ...
            <div className="fu5">
              ...
              <button onClick={doUpdate} disabled={creating||hStatus==='taken'||hStatus==='checking'} className="press" style={{
                width:'100%',padding:'16px',borderRadius:14,
                background:hStatus!=='taken'?grad.css:T.s2,color:'#fff',
                fontSize:16,fontWeight:600,fontFamily:OT,
                boxShadow:hStatus!=='taken'?`0 8px 32px ${grad.sh}`:'none',
                opacity:!creating&&hStatus!=='taken'&&hStatus!=='checking'?1:.45,
              ...
              <div style={{textAlign:'center',marginTop:10,fontSize:11,color:T.t4,lineHeight:1.5}}>
    ...
    const navTabs=[s1ok,s2ok]

    return (
      <div style={{minHeight:'100vh',...bgStyle,fontFamily:OT,position:'relative'}}>
        <Orbs/>
        {AppHeader({
          title: isEditing?'Modifier':'Créer ma carte',
        ...
        {isDesktop?(
          <div style={{display:'flex',minHeight:'100vh',position:'relative',zIndex:1}}>
            <div style={{width:MAX_W,flexShrink:0,overflowY:'auto',height:'100vh',borderRight:`1px solid ${T.sep}`,paddingTop:HDR_H}}>
              {!isEditing&&(
                <div style={{padding:'20px 40px 0',display:'flex',gap:5}}>
                  {navTabs.map((ok,i)=>(
  ...
  if(screen==='auth') return (
    <div style={{minHeight:'100vh',...bgStyle,fontFamily:OT,
      display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
      padding:'40px 24px',position:'relative'}}>
      <Orbs/>
      {AppHeader({back:()=>setScreen(user?'mycard':'landing')})}
      <div style={{width:'100%',maxWidth:400,position:'relative',zIndex:1,paddingTop:HDR_H}}>
        {!authSent?(
          <>
  ...
  if(screen==='mycard'){
    ...
    return (
      <div style={{minHeight:'100vh',...bgStyle,fontFamily:OT,position:'relative'}}>
        <Orbs/>
        {AppHeader({
          navItems: isDesktop?navItems:undefined,
          currentNav: nav,
          onNav: setNav,
          actions:(
            <button onClick={()=>window.open(`/${u.handle}`,'_blank')} style={{
              background:T.t5,border:`1px solid ${T.sep}`,borderRadius:10,padding:'6px 13px',
        ...
        {isDesktop&&(
          <div style={{position:'fixed',top:HDR_H,bottom:0,left:MAX_W,right:0,zIndex:1,
            display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
            padding:'48px',gap:28,borderLeft:`1px solid ${T.sep}`,background:T.bg}}>
            <div style={{width:'100%',maxWidth:380}}><BusinessCard u={{...u,logo:u.logo??undefined}} large floating/></div>
        ...
        {/* Main column */}
        <div style={{maxWidth:MAX_W,position:'relative',zIndex:1,
            ...
            <div style={{padding:'24px 16px'}}>
                ...
                {[
                  ...
                  <div key={s.l} style={{background:T.s1,border:`1px solid ${T.sep}`,borderRadius:14,padding:'15px 12px',textAlign:'center'}}>
                    {s.v!==null
                      ...
                      : <div style={{display:'flex',justifyContent:'center',marginBottom:4}}><Sk w={36} h={24} r={4}/></div>}
                    <div style={{fontFamily:OT,fontSize:10,color:T.t3,marginTop:5,letterSpacing:.4,textTransform:'uppercase'}}>{s.l}</div>
                  </div>
                ))}
              </div>

              {!isDesktop&&<div className="fu2" style={{marginBottom:26}}><BusinessCard u={{...u,logo:u.logo??undefined}} floating/></div>}
              ...
              <div className="fu4" style={{marginBottom:14}}>
                <div style={{position:'relative'}}>
                  ...
                  <button onClick={()=>doShare(u.handle,u.name)} className="press" style={{
          ...
          {nav==='contacts'&&(()=>{
            ...
            return (
              <div style={{padding:'24px 16px'}}>
                ...
                {ctLoaded&&contacts.length>0&&(
                  <div className="fu2" style={{marginBottom:16,display:'flex',gap:8}}>
                    <div style={{flex:1,display:'flex',alignItems:'center',gap:8,background:T.s1,borderRadius:12,padding:'10px 14px',border:`1px solid ${T.sep}`}}>
                      ...
                      <input type="search" placeholder="Rechercher…" value={ctSearch}
                        onChange={e=>{setCtSearch(e.target.value);setCtPage(1)}}
                    ...
                    <div style={{display:'flex',background:T.s1,borderRadius:12,border:`1px solid ${T.sep}`,overflow:'hidden',flexShrink:0}}>
                      {(['date','name'] as const).map(s=>(
                        <button key={s} onClick={()=>setCtSort(s)} style={{padding:'10px 12px',fontSize:12,fontFamily:OT,fontWeight:500,
                          background:ctSort===s?T.s3:'transparent',color:ctSort===s?T.t1:T.t3,
                ...
                {!ctLoaded&&<div style={{display:'flex',flexDirection:'column',gap:10}}>{[1,2,3].map(i=>(
                  <div key={i} style={{display:'flex',alignItems:'center',gap:14,padding:'14px 0'}}>
                    <Sk w={40} h={40} r={13}/><div style={{flex:1,display:'flex',flexDirection:'column',gap:6}}><Sk w="60%" h={14} r={4}/><Sk w="40%" h={11} r={3}/></div>
                  </div>
                ))}</div>}
                {ctLoaded&&contacts.length===0&&<div style={{textAlign:'center',padding:'40px 20px'}}>
                ...
                {ctLoaded&&visible.length>0&&(
                  <>
                      ...
                      {visible.map((c,i,arr)=>{
                        ...
                        return (
                          <Row key={c.contact_handle} last={i===arr.length-1}
                            onTap={()=>window.open(`/${c.contact_handle}`,'_blank')} T={T}>
                            <div style={{display:'flex',alignItems:'center',gap:14,minHeight:58}}>
                              <div style={{width:40,height:40,borderRadius:13,flexShrink:0,background:cG.css,
                                display:'flex',alignItems:'center',justifyContent:'center',
                    ...
                    {hasMore&&<button onClick={()=>setCtPage(p=>p+1)}
          ...
          {nav==='profil'&&(
            <div style={{padding:'24px 16px'}}>
              ...
              <Section label="Compte" T={T}>
                {authUser?(
                  <>
                    <Row T={T}><div style={{display:'flex',alignItems:'center',minHeight:52,justifyContent:'space-between'}}>
                      <div><div style={{fontSize:15,color:T.t1}}>Connecté</div><div style={{fontSize:12,color:T.t3,marginTop:1,fontWeight:300}}>{authUser.email}</div></div>
                      <div style={{width:8,height:8,borderRadius:'50%',background:'#22c55e'}}/>
                    </div></Row>
                    <Row T={T} onTap={startEdit}><div style={{display:'flex',alignItems:'center',minHeight:50,justifyContent:'space-between'}}>
                    ...
                    <Row T={T} onTap={()=>window.open(`/${u.handle}`,'_blank')}><div style={{display:'flex',alignItems:'center',minHeight:50,justifyContent:'space-between'}}>
                  ...
                  <>
                    ...
                    <Row last onTap={()=>{setAuthSent(false);setScreen('auth')}} T={T}><div style={{display:'flex',alignItems:'center',minHeight:50,justifyContent:'space-between'}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',minHeight:50,justifyContent:'space-between'}}>
                      <div>
                        <div style={{display:'flex',alignItems:'center',gap:8}}>
                          ...
                          {it.b&&<span style={{background:g.css,borderRadius:5,padding:'1px 7px',fontSize:9,fontWeight:600,color:'#fff',letterSpacing:.8}}>{it.b}</span>}
                        </div>
              ...
              <div style={{textAlign:'center',padding:'8px 0 4px'}}>
                <div style={{fontFamily:OT,fontSize:11,fontWeight:300,color:T.t4,letterSpacing:.5}}>tapcard · v1.0.0 · one tap. real connection.</div>
              </div>
            </div>
          )}
        </div>

        {/* Tab bar — mobile only — no overlap issue since PWA is in header */}
        {!isDesktop&&(
          <div style={{position:'fixed',bottom:0,left:0,right:0,
            ...
            padding:'11px 0 22px',zIndex:50,transition:'background .25s'}}>
        ...
        {share&&(
          <div className="fi" style={{position:'fixed',inset:0,background:'rgba(0,0,0,.55)',zIndex:100}}>
            ...
            <div className="su" style={{position:'absolute',bottom:0,
              left:isDesktop?'auto':0,right:0,width:isDesktop?MAX_W:'100%',
              ...
              <div style={{flex:1,padding:'0 20px 52px',overflowY:'auto',display:'flex',flexDirection:'column',alignItems:'center'}}>
                ...
                {stab==='link'&&(
                  <div style={{width:'100%',display:'flex',flexDirection:'column',gap:14}}>
                    <div style={{textAlign:'center',padding:'10px 0 4px'}}>
                      ...
                      <div style={{fontFamily:OT,fontSize:13,color:T.t3,lineHeight:1.65,fontWeight:300}}>Partagez par n&apos;importe quel canal.<br/>Aucune installation côté destinataire.</div>
                    </div>
                ...
                {stab==='nfc'&&(
                  <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:22,width:'100%',paddingTop:14}}>
                    <div style={{position:'relative',width:116,height:116,borderRadius:'50%',background:T.s2,border:`1px solid ${T.sep}`,display:'flex',alignItems:'center',justifyContent:'center'}}>
