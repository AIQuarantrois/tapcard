...
export type FilledSocial = { id: string; label: string; color: string }
...
export function getFilledSocials(
  ...
  const soc = socials || {}
  return [
    linkedin?.trim() ? { id:'linkedin', label:'LinkedIn', color:'#0A66C2' } : null,
    ...SOCIALS.filter(s => soc[s.id]?.trim()).map(s => ({ id:s.id, label:s.label, color:s.color })),
